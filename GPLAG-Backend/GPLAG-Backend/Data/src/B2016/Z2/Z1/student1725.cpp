...
double W(std::vector<std::pair<double,double>> parovi , int i , int d) {
    double suma{0};
...
std::function<double (double)> BaricentricnaInterpolacija(std::vector<std::pair<double,double>> parovi, int d) {
    ...
    return [parovi,d](double a) {
        ...
        for(int i=1;i<=parovi.size();i++) 
        {
            ...
            {
                brojnik+=(W(parovi,i,d)/(a-parovi[i-1].first))*parovi[i-1].second;
                nazivnik+=(W(parovi,i,d)/(a-parovi[i-1].first));
            }
...
std::function<double (double)> BaricentricnaInterpolacija(std::function<double(double)> fun,double xmin,double xmax,double delta,int d) {
    ...
    return [cvorovi,d,fun](double a) {
        ...
        for(int i=1;i<=cvorovi.size();i++) 
        {
            ...
            {
                brojnik+=(W(cvorovi,i,d)/(a-cvorovi[i-1].first))*cvorovi[i-1].second;
                nazivnik+=(W(cvorovi,i,d)/(a-cvorovi[i-1].first));
            }
            else return fun(a);
        }
            return brojnik/nazivnik;
...
int main ()
{
    ...
    else 
    {
        ...
        while(1) {
            ...
            try
            {
                std::function<double (double)> f{BaricentricnaInterpolacija([](double a) {return a*a+sin(a);},xmin,xmax,delta,d)};
                double ispis{f(argument)};
                std::cout<<"f("<<argument<<") = "<<argument*argument+sin(argument)<<" fapprox("<<argument<<") = "<<ispis<<std::endl;
            }
            catch(std::domain_error izuzetak)
            {
                std::cout<<izuzetak.what()<<std::endl;
