...
struct Cvor {
    Cvor(Karta ii, std::shared_ptr<Cvor> dd)
        : karta(ii)
        , sljedeci(dd) {
    }
    Karta karta;
...
bool Ispravan (std::shared_ptr<Cvor> &pocetak)
{
...
bool IspravnoSortiran (std::shared_ptr<Cvor> &pocetak)
{
    std::shared_ptr<Cvor> p(pocetak);
    if (!pocetak) return true;
    p=p->sljedeci;
    while (p!=pocetak) {
        std::shared_ptr<Cvor> pom(pocetak);
        while (pom!=p) {
...
void IzbaciJednu (std::shared_ptr<Cvor> &pocetak,std::shared_ptr<Cvor> &prethodni,std::shared_ptr<Cvor> &p)
{
    if (p == pocetak) {
        prethodni=pocetak;
...
std::shared_ptr<Cvor> KreirajSpil()
{
    ...
    for (int i=0; i<52; i++) {
        ...
        auto novi=std::make_shared<Cvor> (Cvor {k,pocetak});
...
void IzbaciKarte (std::shared_ptr<Cvor> &pocetak, std::multimap<Boje, std::string> &karte)
{
    ...
    std::shared_ptr<Cvor> p=pocetak;
    std::shared_ptr<Cvor> pomocna(pocetak);
    int brojac(0);
    do {
        brojac++;
...
std::stack<std::pair<std::string,std::string>> IzbaciKarteRazbrajanjem (std::shared_ptr<Cvor> &pocetak,const short int &korak,const int &broj)
{
...
void VratiPosljednjihNKarata (std::shared_ptr<Cvor> &pocetak, std::stack<std::pair<std::string,std::string>> &bacene, int n)
{
    ...
    while (brojac!=n) {
        ...
        if (bacene.top().first=="Karo")
            k.boja=Boje::Karo;
        k.vrijednost=bacene.top().second;
        auto novi (std::make_shared<Cvor> (Cvor {k, p}));
...
void IspisiSpil (std::shared_ptr<Cvor> pocetak)
{
    ...
    int brojac2(0);
    if (pocetak!=nullptr)
        for (std::shared_ptr<Cvor> p=pocetak; p->sljedeci!=pocetak; p=p->sljedeci) brojac++;
    std::cout<<"Pik: ";
    if (pocetak!=nullptr)
...
int main ()
{
    std::shared_ptr<Cvor> pocetak (KreirajSpil());
    std::shared_ptr<Cvor> p;
    try {
        short int n;
        int r;
