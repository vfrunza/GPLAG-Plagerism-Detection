...
typedef std::shared_ptr<Cvor> Pampok;

void IspisiSpil(Pampok spil)
{
    int broj_karata(1);
    Pampok prva(spil);
...
Pampok KreirajSpil()
{
    ...
    for(int i(0); i<52; i++) {
        ...
        if(j==13) broj="K";
        if(j==14) broj="A";
        Pampok nova(std::make_shared<Cvor>());
...
void IzbaciKarte(Pampok &prva, std::multimap<Boje, std::string> mapa)
{
    int broj_karata(1);
    Pampok pom(prva->sljedeci);
...
std::stack<std::pair<std::string, std::string>> IzbaciKarteRazbiranjem(Pampok &prva, const short int &korak_razbrajanja, const int &broj_karata)
{
...
void VratiPosljednjihNKarata(Pampok &spil, std::stack<std::pair<std::string, std::string>> &stek_karata, int n)
{
    if(n<0) throw std::domain_error("Broj n je besmislen!");
    if(n>stek_karata.size()) throw std::range_error("Nedovoljno karata u redu!");
...
void UnistiSpil(Pampok spil)
{
    if(spil==nullptr) return;
    Pampok pom(spil->sljedeci);
    int broj_karata(1);
