...
void IzbaciKarte(Deklaracija& lista, std::multimap
                 <Boje,std::string>& mapa)
{
    try {
        if (!Provjera(lista)) throw std::logic_error("Neispravna lista!"); 
        for (auto itMAPE=mapa.begin(); itMAPE!=mapa.end(); itMAPE++) {
            for (auto itLISTE=lista.begin(); itLISTE!=lista.end(); itLISTE++) {
                if (itMAPE->first==itLISTE->first && itMAPE->second==itLISTE->second) {
                    for (auto it=lista.begin(); it!=lista.end(); it++) {
                        /*Brisanje para iz LISTE*/ if (it->first==itLISTE->first
                                                       && it->second==itLISTE->second) {
                            it=lista.erase(it);
                        }
                    }
...
std::queue<std::pair<std::string,std::string>> IzbaciKarteRazbrajanjem (Deklaracija &lista, const short int& r, const int& b )
{
...
void VratiPrvihNKarata (Deklaracija& lista,std::queue<std::pair<std::string,std::string>>& red, int n)
{
    try {
