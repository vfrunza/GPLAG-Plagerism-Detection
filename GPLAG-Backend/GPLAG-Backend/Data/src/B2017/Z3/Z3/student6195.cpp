...
Matrica<TipElemenata> JedinicnaMatrica (int br_redova, int br_kolona) {
    auto m = StvoriMatricu<TipElemenata>(br_redova, br_kolona);
...
Matrica<TipElemenata> MatricniPolinom (const Matrica<TipElemenata> &m, int n, IterTip p) {
    ...
    auto finalna = JedinicnaMatrica<TipElemenata>(m.br_redova, m.br_kolona);
    for(int i = 0; i <= n; i++) {
        if(i == 0) {
            for(int j = 0; j < finalna.br_redova; j++)
                for(int k = 0; k < finalna.br_kolona; k++)  finalna.elementi[j][k] *= *p ;
            p++;
        }else{
            if(i == 1) {
                ...
                auto mat_A = m;
                for(int j = 0; j < finalna.br_redova; j++)
                    for(int k = 0; k < finalna.br_kolona; k++)  finalna.elementi[j][k] += (*p) * mat_A.elementi[j][k]; 
                p++;
               
            }else{
                auto mat_A = ProduktMatrica(m,m);
                int brojac = 2;
                while(brojac < i) {
                    auto mat_b=mat_A;
                    mat_A = ProduktMatrica(mat_A, m);
                    brojac++;
                    UnistiMatricu(mat_b);
                }
                for(int j = 0; j < finalna.br_redova; j++)
...
int main ()
{
    ...
    try {
        ...
        try{
            IspisiMatricu(MatricniPolinom(a, n, kontejner.begin()), 10, 6, true);
        	}catch(std::bad_alloc) {
