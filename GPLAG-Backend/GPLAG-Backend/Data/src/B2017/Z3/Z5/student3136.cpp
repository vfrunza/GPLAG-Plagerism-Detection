...
int VelicinaListe(Cvor *pocetak)
{
    auto kopija(pocetak);
    int brojac(0);
    while(kopija->sljedeci!=pocetak) {
...
Cvor* Nadji(Cvor *pocetak,std::pair<Boje,std::string>par)
{
    Cvor *rez=pocetak; //novi pokazivac na pocetak
...
Cvor* DajPrethodni(Cvor *pocetak,Cvor *trazeni)
{
    Cvor *dalje=pocetak;
...
void UnistiSpil(Cvor *pocetak)
{
    ...
    auto kraj = pocetak;
    while(kraj->sljedeci != pocetak) {
        kraj = kraj->sljedeci;
    }
    while(pocetak != pocetak->sljedeci) {
        auto brisi = pocetak;
        pocetak = pocetak->sljedeci;
...
std::stack<std::pair<string,string>> IzbaciKarteRazbrajanjem(Cvor *&pocetak,const short int &r,const int &b)
{
...
void VratiPosljednjihNKarata(Cvor *&pocetak,std::stack<std::pair<string,string>> &stek,int n)
{
