...
Cvor *KreirajSpil(){
    ...
    for(int i=0;i<52;i++){
        ...
        Cvor *novi{new Cvor{{bo,imm},nullptr}};
        if(!prvi)prvi=novi;
...
Cvor *izbrisi(Cvor *&p, Cvor *prije,Cvor *pocetak){
    if(sizee(p)==2){
        pocetak=prije;
        prije->veza=nullptr;
        auto d(p);
...
void IzbaciKarte(Cvor *&temp,std::multimap<Boje, std::string> mapa){
...
std::stack<std::pair<Boje,std::string>> IzbaciKarteRazbrajanjem(Cvor *&p,const int &r,const int &n){
...
void ubaci(Cvor *p,Cvor *prije,Cvor *novi){
    novi->veza=p;
    prije->veza=novi;
}
void zamjeni(Cvor *&p,Cvor *&prije,Cvor *&poslije,Cvor *&p2,Cvor *&prije2,Cvor *&poslije2){
    prije->veza=p2;
    p2->veza=poslije;
    prije2->veza=p;
    p->veza=poslije2;
...
void VratiPosljednjihNKarata(Cvor *&p,std::stack<std::pair<Boje,std::string>> stek,int n){
        if(n<0){UnistiSpil(p);throw std::domain_error {"Broj n je besmislen!"};}
       if(n>stek.size()){UnistiSpil(p);throw std::range_error{"Nedovoljno karata u steku!"};}
       if(!(je_li_validan(p))){UnistiSpil(p);throw std::logic_error{"Neispravna lista!"};}
       if(!je_li_validan(p)){UnistiSpil(p);throw std::logic_error{"Neispravna lista!"};}
