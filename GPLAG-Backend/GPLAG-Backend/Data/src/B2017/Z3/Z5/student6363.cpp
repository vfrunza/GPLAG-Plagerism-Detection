...
void UnistiSpil(Cvor *c){
    Cvor *p=c;
    do {
        Cvor *temp {nullptr};
        if(p!=nullptr) temp=p->sljedeci;
        delete p;
        p=temp;
    } while(p!=c && p!=nullptr);
}
...
Cvor* KreirajSpil() {
    ...
    while(TipKarte<=3) {
        for(int i=0; i<Karte.size(); i++) {
            Cvor *novi {nullptr};
            try {
                novi = new Cvor {{static_cast<Boje>(TipKarte),Karte.at(i)},nullptr};
...
Cvor * DajPrethodni(Cvor*pocetak,Cvor *c) {
    Cvor *p=pocetak;
...
bool ispravnoSortiranSpil(Cvor *c) {
    if(!c) return true;
    Cvor *p=c;
...
void IzbaciKarte(Cvor *&c, MMapa &m) {
...
std::stack<pair<string,string>> IzbaciKarteRazbrajanjem(Cvor *&c,const short int &r, const int &b) {
    ...
    while(brojac!=0) {
        short int i = r-1;
        Cvor *prethodni=DajPrethodni(c,p);
...
bool Prisutan(Cvor *c, pair<Boje,string> x) {
    if(!c) return false;
    Cvor *p=c;
    do {
...
void Podesi(Cvor *&c, pair<Boje,string> x)
{
    Cvor *novi = new Cvor {{x.first,x.second},nullptr};
    int velicina=PrebrojSpil(c);
    if(velicina==0) {
        c=novi;
        return;
...
void VratiPosljednjihNKarata(Cvor *&c, std::stack<pair<string,string>> s, int n) {
    ...
    /* Cvor *spil {nullptr};
     try {
         ...
         MMapa mapa {{Boje::Pik,"2"},{Boje::Pik,"3"},{Boje::Pik,"4"},{Boje::Pik,"5"},{Boje::Pik,"6"},{Boje::Pik,"7"},
             ...
             {Boje::Karo,"8"},{Boje::Karo,"9"},{Boje::Karo,"10"},{Boje::Karo,"J"},{Boje::Karo,"Q"},{Boje::Karo,"K"}
         };
         IzbaciKarteRazbrajanjem(spil,2,52);
         IspisSpila(spil);
     } catch(std::exception &ex) {
