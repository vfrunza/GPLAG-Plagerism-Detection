...
Cvor *KreirajSpil()
{
    ...
    for (int i{};i<52;i++)
        {
            Cvor *novi = new Cvor;                                              ///Kreiranje i popunjavanje cvorova.
...
std::stack<std::pair<std::string,std::string>> IzbaciKarteRazbrajanjem (Cvor *&spil, const int &x, const int &b)
{
    ...
    Cvor *p = spil;                                                             ///Pronalazenje broja elemenata u spilu.
    p = p->sljedeci;
    int n{1};
    for (p ; p!=spil; p = p->sljedeci)
        n++;
...
void IzbaciKarte (Cvor *&spil, std::multimap<Boje, std::string> &mapa)
{
...
void VratiPosljednjihNKarata (Cvor *&spil, std::stack<std::pair<std::string,std::string>> &ubacivanje, int n)
{
    if (n < 0)
