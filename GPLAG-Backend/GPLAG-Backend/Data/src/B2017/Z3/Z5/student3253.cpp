...
void IzbaciKarte(Cvor* &spil, std::multimap<Boje, std::string> &crnaLista) {
...
void VratiPosljednjihNKarata(Cvor *spil, std::stack<std::pair<std::string, std::string>> &red, int n){
...
void UnistiSpil(Cvor *spil){
    ...
    while(obrisano < spilSize){
        // std::cout << "Brisem" << dajStringBoje(trenutniCvor->karta.boja) << " " << trenutniCvor->karta.vrijednost << std::endl;
        prethodniCvor = trenutniCvor;
        trenutniCvor = trenutniCvor->sljedeci;
