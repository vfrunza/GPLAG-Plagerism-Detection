...
void IzbaciKarte(Cvor* &karte,multimap<Boje,string> &mapa)
{
     int br=0;//Prebroj cvorove
...
stack<pair<Boje,string>> IzbaciKarteRazbrajanjem(Cvor* &karte,const short int &r,int b)
{   
    int pom=r;
    auto it=karte;
...
void VratiPosljednjihNKarata(Cvor* karte,stack<pair<Boje,string>> stek,int n)
{   
...
void UnistiSpil(Cvor* karte)
{
    Cvor* tren=karte->sljedeci;
    Cvor* temp=karte;
    while(tren->sljedeci!=karte)
    {
        delete temp;
        temp=tren;
        tren=tren->sljedeci;
