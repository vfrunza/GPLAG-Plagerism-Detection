...
void Test() const {
 ...
 ~PolimorfniSpremnik() {}
 PolimorfniSpremnik(const Spremnik &lik) : pok(std::shared_ptr<Spremnik> (lik.DajKopiju())) {}
PolimorfniSpremnik(const PolimorfniSpremnik &lik) {
 if(!lik.pok) pok = nullptr;
 else pok = std::shared_ptr<Spremnik>(lik.pok->DajKopiju());
 }
 PolimorfniSpremnik(PolimorfniSpremnik &&lik) {
 pok = lik.pok; lik.pok = nullptr;
 }
 PolimorfniSpremnik &operator =(const PolimorfniSpremnik &lik) {
 std::shared_ptr<Spremnik> p_novi(nullptr);
 if(lik.pok != nullptr) p_novi = std::shared_ptr<Spremnik> (lik.pok->DajKopiju());
 pok = p_novi;
 return *this;
 }
 double DajTezinu() const {
     Test(); return pok->DajTezinu();
 }
 double DajUkupnuTezinu() const {
     Test(); return pok->DajUkupnuTezinu();
 }
 PolimorfniSpremnik &operator =(PolimorfniSpremnik &&lik) {
 std::swap(pok, lik.pok);
 return *this;
 }
