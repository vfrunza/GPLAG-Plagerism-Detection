// Zadatak2.cpp : Defines the entry point for the console application.
//
...
public:
	...
	PolimorfniSpremnik(const PolimorfniSpremnik &p) { spremnik = p.spremnik->DajKopiju(); }
	PolimorfniSpremnik(PolimorfniSpremnik &&p) : spremnik(p.spremnik) { p.spremnik = nullptr; }
	PolimorfniSpremnik(const Spremnik &s) { spremnik = s.DajKopiju(); }
	PolimorfniSpremnik(Spremnik &&s) { spremnik = s.DajKopiju(); }
	PolimorfniSpremnik& operator =(const PolimorfniSpremnik &p) { spremnik = p.spremnik->DajKopiju(); return *this; }
	PolimorfniSpremnik& operator =(PolimorfniSpremnik &&p) { std::swap(p.spremnik, spremnik); return *this; }
	PolimorfniSpremnik& operator =(const Spremnik &s) { spremnik = s.DajKopiju(); }
	PolimorfniSpremnik& operator =(Spremnik &&s) { spremnik = s.DajKopiju(); }
