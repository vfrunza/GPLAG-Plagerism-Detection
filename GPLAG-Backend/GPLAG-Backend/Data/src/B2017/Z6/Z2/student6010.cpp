...
class PolimorfniSpremnik {
    Spremnik* pok;
    void Test() const {
        if(!pok) throw logic_error("Nespecificiran spremnik");
    }
    public:
    PolimorfniSpremnik(): pok(nullptr) {}
    ~PolimorfniSpremnik() {delete pok;}
   // PolimorfniSpremnik(const PolimorfniSpremnik &s): pok(make_shared(s->Kopija())) {}
    PolimorfniSpremnik &operator=(const PolimorfniSpremnik &s) {
        Spremnik *novipok=nullptr;
        if(s.pok!=nullptr)  novipok=s.pok->Kopija();
        delete pok;
        pok=novipok;
        return *this;
    }
    double DajUkupnuTezinu() const { Test(); return pok->DajUkupnuTezinu(); }
    void Ispisi() const {Test(); return pok->Ispisi(); }
