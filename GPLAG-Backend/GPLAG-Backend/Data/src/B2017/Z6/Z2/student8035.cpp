...
public:
    ...
    PolimorfniSpremnik &operator=(const PolimorfniSpremnik &ps)
    {
        ...
        if(ps.p != nullptr) p_novi=ps.p->DajKopiju();
        delete p;
        p=p_novi;
        return *this;
    }
    ...
    double DajTezinu() const { Test(); return p->DajTezinu(); }
    double DajUkupnuTezinu() const { Test(); return p->DajUkupnuTezinu(); }
    void Ispisi() const { Test(); p->Ispisi(); }
    Spremnik *DajKopiju() const { Test(); return p->DajKopiju(); }
};
