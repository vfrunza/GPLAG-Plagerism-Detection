...
Kugla &PostaviX(double x1){x=x1;return *this;}
Kugla &PostaviY(double y1){y=y1;return *this;}
Kugla &PostaviZ(double z1){z=z1;return *this;}
Kugla &PostaviCentar(double x1, double y1, double z1){x=x1;y=y1;z=z1;return *this;}
...
Kugla &PostaviPoluprecnik(double r1){r=r1;return *this;}
...
bool DaLiSadrzi(const Kugla &k2) const{
...
bool DaLiSuPodudarne(const Kugla &k1, const Kugla &k2){if(k1.r==k2.r)return true; if(k1.r+epsilon==k2.r-epsilon)return true;if(k1.r-epsilon==k2.r+epsilon)return true;return false; }
bool DaLiSuKoncentricne(const Kugla &k1, const Kugla &k2){if(k1.x==k2.x&&k1.y==k2.y&&k1.z==k2.z)return true;if(k1.x+epsilon==k2.x-epsilon&&k1.y+epsilon==k2.y-epsilon&&k1.z+epsilon==k2.z-epsilon)return true;if(k1.x-epsilon==k2.x+epsilon&&k1.y-epsilon==k2.y+epsilon&&k1.z-epsilon==k2.z+epsilon)return true;return false;}
...
int main (){
    ...
    for(int i=0;i<n;i++){
        ...
        v[i]=std::make_shared<Kugla>(a1,a2,a3,a4);
    }
    std::cout << "Unesite parametre translacije (delta_x,delta_y,delta_z): ";
    double dex,dey,dez;
    std::cin>>dex>>dey>>dez;
    std::vector<std::shared_ptr<Kugla>> kys(n);
    std::cout << "Kugle nakon obavljenje transformacije imaju sljedece vrijednosti: \n";
    std::transform(v.begin(),v.end(),kys.begin(),[ dex, dey, dez](std::shared_ptr<Kugla> k){k->Transliraj(dex,dey,dez);return k;});
    std::sort(kys.begin(),kys.end(),[](std::shared_ptr<Kugla> k1,std::shared_ptr<Kugla> k2){return k1->DajZapreminu()<k2->DajZapreminu();});
    std::for_each(kys.begin(),kys.end(),[](std::shared_ptr<Kugla>k){k->Ispisi();});
    std::cout << "Kugla sa najvecom povrsinom je: ";
auto p=std::max_element(kys.begin(),kys.end(),[](std::shared_ptr<Kugla>k1,std::shared_ptr<Kugla>k2){return k1->DajPovrsinu()<k2->DajPovrsinu();});
	(*p)->Ispisi();
	int e=0;
	int br=0;
	std::for_each(kys.begin(),kys.end(),[kys,&e,&br](std::shared_ptr<Kugla> k1){
	    std::for_each(kys.begin()+e,kys.end(),[k1,&br](std::shared_ptr<Kugla> k2){
	        if(k2==k1);else{
