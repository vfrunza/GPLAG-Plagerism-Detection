...
public:
     ...
     Kugla &PostaviY(double y) { yy=y; return *this;}
     Kugla &PostaviZ(double z) { zz=z; return *this;}
     ...
     friend bool DaLiSeDodirujuIzvana(const Kugla &k1, const Kugla &k2){
          double veci,manji;
          if(k1.DajPoluprecnik()<=k2.DajPoluprecnik()) { veci=k2.DajPoluprecnik(); manji=k1.DajPoluprecnik();}
          else { veci=k1.DajPoluprecnik(); manji=k2.DajPoluprecnik(); }
          return jednako(veci+manji,RastojanjeCentara(k1,k2));
     }
     ...
     friend bool DaLiSePreklapaju(const Kugla &k1, const Kugla &k2){
           double p=0;
           return jednako(RastojanjeCentara(k1,k2),p);
     }
     friend bool DaLiSeSijeku(const Kugla &k1, const Kugla &k2){
          double veci,manji;
          if(k1.DajPoluprecnik()<=k2.DajPoluprecnik()) { veci=k2.DajPoluprecnik(); manji=k1.DajPoluprecnik();}
          else { veci=k1.DajPoluprecnik(); manji=k2.DajPoluprecnik(); }
          return (veci+manji>RastojanjeCentara(k1,k2) && veci-manji<RastojanjeCentara(k1,k2));
     }
...
int main()
{
    /* try {
     ...
     for(int i=0; i<n; i++){
          ...
          cin>>x>>y>>z;
          while(!cin){
               cin.clear(); cin.ignore(3,'\n'); cout<<"Neispravan centar"<<endl; cout<<"Unesite centar "<<i+1<<". kugle: ";cin>>x>>y>>z;;
          }
          //if(!cin) { cin.clear(); cin.ignore(3,'\n'); cout<<"Neispravan centar"<<endl; i--; continue;}
          //cin>>y;
          //if(!cin) { cin.clear(); cin.ignore(2,'\n'); cout<<"Neispravan centar"<<endl; i--; continue;}
          //cin>>z;
          //if(!cin) { cin.clear(); cin.ignore(1,'\n'); cout<<"Neispravan centar"<<endl; i--; continue;}
          cout<<"Unesite poluprecnik "<<i+1<<". kugle: ";
          double r;
          cin>>r;
          if(!cin) { cin.clear(); cin.ignore(1,'\n'); cout<<"Ilegalan poluprecnik"<<endl; i--; continue;}
          v[i]= make_shared<Kugla>(x,y,z,r);
         //v[i]->Ispisi();
          } catch(domain_error d) { cout<<d.what()<<endl; i--;}
     }
     double x,y,z; cout<<"Unesite parametre translacije (delta_x,delta_y,delta_z): ";
     while(1){
     cin>>x>>y>>z;
     if(!cin) { cin.clear(); cin.ignore(1,'\n'); cout<<"Neispravni parametri translacije, unesite ponovo!"<<endl; continue;}
     break;
     }
     transform(v.begin(), v.end(), v.begin(), [x,y,z](shared_ptr<Kugla> k){
          k->Transliraj(x,y,z);return k;});
     ...
     cout<<"Kugla sa najvecom povrsinom je: ";(max_element(v.begin(),v.end(),[](shared_ptr<Kugla> k1,shared_ptr<Kugla> k2){return k1->DajPovrsinu()<k2->DajPovrsinu();}))->get()->Ispisi();
     bool a=false;
     vector<shared_ptr<Kugla>>::iterator it=v.begin();
     vector<shared_ptr<Kugla>>::iterator it1=v.end();
     for_each(v.begin(),v.end(),[&it,it1,&a](shared_ptr<Kugla> k){
          it++;
          for_each(it,it1,[k,&a](shared_ptr<Kugla> k1){
               if (DaLiSeSijeku(*k,*k1)){a=true; cout<<"Presjecaju se kugle: ";k->Ispisi();k1->Ispisi();}});});
	if(!a)cout<<"Ne postoje kugle koje se presjecaju!"<<endl; 
