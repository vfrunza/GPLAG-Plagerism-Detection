...
public:
    explicit Pregledi ( void ) {}
    
    Pregledi ( std::initializer_list < Pregled > spisak_pregleda )
    {
        for ( Pregled pregled : spisak_pregleda )
            p.push_back ( std::make_shared < Pregled > 
                ( pregled.DajImePacijenta (), pregled.DajDatumPregleda (), pregled.DajVrijemePregleda () ) );
    }
    
    ~Pregledi ( void ) { p.resize ( 0 ); }
    ...
    Pregledi ( Pregledi &&pregledi ) : p { pregledi.p }
        { pregledi.~Pregledi (); }   
    ...
    {
        p.push_back ( std::make_shared < Pregled > ( ime_pacijenta, datum_pregleda, vrijeme_pregleda ) );
    }
    ...
    {
        p.push_back ( std::make_shared < Pregled > ( ime_pacijenta, dan_pregleda, mjesec_pregleda, godina_pregleda, sati_pregleda, minute_pregleda ) );
    }
    
    void RegistrirajPregled ( std::shared_ptr < Pregled > pregled )
    {
        p.push_back ( std::make_shared < Pregled > ( pregled -> DajImePacijenta (), pregled -> DajDatumPregleda (), pregled -> DajVrijemePregleda () ) );
    }
    
    int DajBrojPregleda ( void ) const { return p.size (); }
    int DajBrojPregledaNaDatum ( const Datum &datum ) const
    {
        return std::count_if ( p.begin (), p.end (), 
            [ = ] ( std::shared_ptr < Pregled > p ) -> bool
            {
    ...
    Pregled &DajNajranijiPregled ( void )
    {
        ...
        return **std::min_element ( this -> p.begin (), this -> p.end (),
            [] ( std::shared_ptr < Pregled > p1, std::shared_ptr < Pregled > p2 ) -> bool
    ...
    void IsprazniKolekciju ( void ) { this -> ~Pregledi (); }
    ...
    {
        std::vector < std::shared_ptr < Pregled > > pp;
        for ( int i = 0; i < p.size (); i++ )
            pp.push_back ( std::make_shared < Pregled > ( *p [ i ] ) );
        std::sort ( pp.begin (), pp.end (),
            [] ( std::shared_ptr < Pregled > p1, std::shared_ptr < Pregled > p2 ) -> bool
                { return Pregled::DolaziPrije ( *p1, *p2 ); }
        );
        for ( std::shared_ptr < Pregled > pre : pp )
            if ( pre -> DajDatumPregleda ().Ocitaj () == datum.Ocitaj () )
                { pre -> Ispisi (); std::cout << std::endl; }
    }
...
int main ( void )
{
    ...
    while ( std::cout << "\nOdaberite jednu od sljedecih opcija\n"
        ...
        "7 - Kraj programa\n" )
    {
        int pre_pre { pregledi.DajBrojPregleda () };
        std::cin >> izbor;
        if ( izbor == 7 )
            break;
        switch ( izbor )
