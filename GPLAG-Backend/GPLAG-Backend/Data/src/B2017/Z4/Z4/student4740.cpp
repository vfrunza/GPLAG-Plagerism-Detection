...
class Pregledi{
  std::vector<std::shared_ptr<Pregled>> elementi;
  public: 
  ...
  int DajBrojPregleda() const{return elementi.size();};
  ...
  Pregledi::Pregledi(std::initializer_list<Pregled> spisak_pregleda): 
   elementi(spisak_pregleda.size()){
      auto it(spisak_pregleda.begin());
         for(int i=0;i<spisak_pregleda.size();i++){
          elementi[i]=std::make_shared<Pregled>(*it);
          it++;
  }}

  Pregledi::Pregledi (const Pregledi &pregledi): elementi(pregledi.elementi.size()) {
          for(int i=0;i<elementi.size();i++) elementi[i]=std::make_shared<Pregled>(*pregledi.elementi[i]);
  }
 
  Pregledi &Pregledi::operator= (const Pregledi &pregledi){
      if(&pregledi==this) return *this;
      elementi.resize(pregledi.elementi.size());
          for(int i(0);i<pregledi.elementi.size();i++) elementi[i]=std::make_shared<Pregled>(*pregledi.elementi[i]);
          return *this;
  }
...
int Pregledi::DajBrojPregledaNaDatum(const Datum &datum) const{
        ...
        br=std::count_if(elementi.begin(), elementi.end(), [d2,m2,g2](std::shared_ptr<Pregled> p){
  ...
  Pregled &Pregledi::DajNajranijiPregled(){
   std::shared_ptr<Pregled> najmanji=*std::min_element(elementi.begin(), elementi.end(),[](std::shared_ptr<Pregled> p1, std::shared_ptr<Pregled> p2){
       return Pregled::DolaziPrije(*p1,*p2); });
    return *najmanji;
  }
  
  Pregled Pregledi::DajNajranijiPregled() const{
      if(elementi.size()==0) throw std::range_error("Nema registriranih pregleda");
   std::shared_ptr<Pregled> najmanji=*std::min_element(elementi.begin(), elementi.end(),[](std::shared_ptr<Pregled> p1, std::shared_ptr<Pregled> p2){
       return Pregled::DolaziPrije(*p1,*p2); });
    return *najmanji;
  }
 ...
 void Pregledi::ObrisiNajranijiPregled(){
    std::shared_ptr<Pregled> najmanji=*std::min_element(elementi.begin(), elementi.end(),[](std::shared_ptr<Pregled> p1, std::shared_ptr<Pregled> p2){
       ...
       for(int i(0);i<elementi.size();i++){
           if(elementi[i]==najmanji){
              najmanji=nullptr;
              for(int j=i;j<elementi.size()-1;j++){
                 elementi[j]=elementi[j+1];
              }
          elementi[elementi.size()-1]=nullptr;
       elementi.resize(elementi.size()-1);}}
  ...
  void Pregledi::ObrisiPregledePacijenta(const std::string &ime_pacijenta){
          for(int i(0); i<elementi.size(); i++){
              ...
              elementi[i]=nullptr;
            for(int k(i);k<elementi.size()-1;k++)   elementi[k]=elementi[k+1];
          elementi[elementi.size()-1]=nullptr;
                elementi.resize(elementi.size()-1);
                i--;
              }
  ...
  void Pregledi::IspisiPregledeNaDatum(const Datum &datum) const{
      std::vector<std::shared_ptr<Pregled>> v(elementi.size());
      for(int i(0);i<elementi.size();i++) v[i]=elementi[i];
      std::sort(v.begin(), v.end(), [](std::shared_ptr<Pregled> p1, std::shared_ptr<Pregled> p2){return Pregled::DolaziPrije(*p1,*p2);});
      std::tuple<int,int,int> datum2(datum.Ocitaj());
      for(int i(0);i<elementi.size();i++){
  ...
  void Pregledi::IspisiSvePreglede() const{
      ...
      for(int i(0);i<elementi.size();i++) v[i]=elementi[i];
      std::sort(v.begin(), v.end(), [](std::shared_ptr<Pregled> p1, std::shared_ptr<Pregled> p2){return Pregled::DolaziPrije(*p1,*p2);});
      for(int i(0);i<elementi.size();i++) v[i]->Pregled::Ispisi();
  }
int main ()
{
