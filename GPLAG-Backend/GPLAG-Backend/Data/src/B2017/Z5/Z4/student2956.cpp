...
public:
    FourierovRed(double T, std::initializer_list<double> lista1, std::initializer_list<double> lista2) : T(T){
        ...
        for(auto i : lista1){ a[brojac][0] = i; brojac++; }
        brojac = 1;
        for(auto i : lista2){ b[brojac][0] = i; brojac++; }
    }
    ...
    FourierovRed(int N, double p, double q, prvi funkcija1, int M){
        ...
        for(int i = 0; i < N; i++){
            ...
            a[i][0] = (funkcija1(p) + funkcija1(q)) / M * cos(2 * double(i) * pi * p / T) + 2. / M * pomocna1;
            pomocna1 = 0;
            for(int j = 1; j <= M - 1; j++){
                pomocna1 += funkcija1(p + double(j) * T / M) * sin(2 * double(i) * pi * (p / T + double(j) / M));
            }
            b[i][0] = (funkcija1(p) + funkcija1(q)) / M * sin(2 * double(i) * pi * p / T) + 2. / M * pomocna1;
            pomocna1 = 0;
        }
    }
    ...
    FourierovRed(const FourierovRed& staro){
        ...
        (*this).b = new double*[N]{};
        for(int i = 0; i < N; i++){
            (*this).a[i] = new double(staro.a[i][0]);
            (*this).b[i] = new double(staro.b[i][0]);
        }
    }
    ...
    double operator ()(double x){
        double vrijednost = (*this).a[0][0] / 2.;
        double pomocna(0);
        for(int i = 1; i < (*this).N; i++){
            pomocna += a[i][0] * cos(2 * double(i) * pi * x / (*this).T) + b[i][0] * sin(2 * double(i) * pi * x / (*this).T);
        }
        return (vrijednost + pomocna);
    }
    std::pair<double, double> operator [](int k){
        if(k < 0 || k + 1> (*this).N) throw std::range_error("Neispravan indeks");
        if(k == 0){ std::shared_ptr<std::pair<double, double>> par; par = std::make_shared<std::pair<double, double>>(std::make_pair((*this).a[k][0], 0.)); return *par; } 
        std::shared_ptr<std::pair<double, double>> par; par = std::make_shared<std::pair<double, double>>(std::make_pair((*this).a[k][0], (*this).b[k][0])); return *par;
    }
};
