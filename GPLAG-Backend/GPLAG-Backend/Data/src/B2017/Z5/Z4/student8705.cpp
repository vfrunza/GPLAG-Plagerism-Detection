...
class FourierovRed
{
    double *a{}, *b{}, T;
    int n;
    std::pair<double*, double*> Alociraj(int N);
...
std::pair<double*, double*> FourierovRed::Alociraj(int n)
{
    std::pair<double*, double*> p;
    p.first = new double[n+1]{};
    try { p.second = new double[n+1]{}; } catch (...) { delete[] p.first; throw; }
    return p;
}
...
: T(T)
{
    std::pair<double*&, double*&>(a, b) = Alociraj(n = std::max(int(_a.size()) - 1, int(_b.size())));
    int i(0);
...
: T(T)
{
    ...
    std::pair<double*&, double*&>(a, b) = Alociraj(n = N);
    for (int i(0); i <= n; ++i)
    {
        a[i] = f(i);
        if (i > 0) b[i] = g(i);
...
{
    if (&f == this) return;
    auto novi(Alociraj(f.n));
    std::copy(f.a, f.a+f.n+1, novi.first);
    std::copy(f.b, f.b+f.n+1, novi.second);
    delete[] a; delete[] b;
    a = novi.first; b = novi.second;
    n = f.n; T = f.T;
}
