...
{
    std::map<int, shared_ptr<Laptop>> laptopi;
    std::map<int, shared_ptr<Student>> studenti;
public:
...
Administracija::Administracija(const Administracija &a)
{
    ...
    for (auto &x : a.laptopi)
        laptopi[x.first] = make_shared<Laptop>(*x.second);
    for (auto &x : a.studenti)
        studenti[x.first] = make_shared<Student>(*x.second);
}
void Administracija::RegistrirajNovogStudenta(int i, const string &g, const string &im, const string &adr, const string &tel)
{
    ...
    studenti[i] = make_shared<Student>(i, g, im, adr, tel);
}
void Administracija::RegistrirajNoviLaptop(int br, const string &n, const string &k)
{
    ...
    laptopi[br] = make_shared<Laptop>(br, n, k);
}
...
void Administracija::ZaduziLaptop(int ind, int br)
{
    ...
    if (std::find_if(laptopi.begin(), laptopi.end(), [&](const std::pair<int, shared_ptr<Laptop>> &p) {
...
{
    auto it(std::find_if(laptopi.begin(), laptopi.end(), [](const std::pair<int, shared_ptr<Laptop>> &p) {
