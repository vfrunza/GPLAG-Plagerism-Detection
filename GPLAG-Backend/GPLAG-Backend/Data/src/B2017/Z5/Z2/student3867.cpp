...
public:
    ...
    void Zaduzi ( Student &s )
    {
        ...
        kod_koga_je = std::make_shared < Student > ( s.DajIndeks (), s.DajGodinuStudija (), s.DajImePrezime (), s.DajAdresu (), s.DajTelefon () );
    ...
    Student *DajPokKodKogaJe ( void ) const { return kod_koga_je.get (); }
...
public:
    ...
    {
        for ( const std::pair < const int, std::shared_ptr < Student > > &p : admin.studenti )
            studenti [ p.first ] = std::make_shared < Student > 
                ( p.second -> DajIndeks (), p.second -> DajGodinuStudija (), 
                p.second -> DajImePrezime (), p.second -> DajAdresu (), p.second -> DajTelefon () );
                
        for ( const std::pair < const int, std::shared_ptr < Laptop > > &p : admin.laptopi )
            laptopi [ p.first ] = std::make_shared < Laptop > 
    ...
    void RegistrirajNovogStudenta ( int index, std::string godina, std::string ime, std::string adresa, std::string telefon )
    {
        ...
        studenti [ index ] = std::make_shared < Student > ( index, godina, ime, adresa, telefon );
    }
    ...
    {
        int i { 0 };
        for ( const std::pair < const int, std::shared_ptr < Student > > p : studenti )
        {
            p.second -> Ispisi ();
    ...
    {
        int i { 0 };
        for ( const std::pair < const int, std::shared_ptr < Laptop > > p : laptopi )
        {
            p.second -> Ispisi ();
