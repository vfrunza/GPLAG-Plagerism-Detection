...
class Administracija
{
    ...
    Administracija(const Administracija &a)
    {
        ...
        {
            studenti.insert(std::make_pair<int, std::shared_ptr<Student>>(std::remove_reference<int&>::type(it->first), std::make_shared<Student>(it->first, (it->second)->DajGodinuStudija(), (it->second)->DajImePrezime(), (it->second)->DajAdresu(), (it->second)->DajTelefon())));
        }
        for(auto it1(a.laptopi.begin()); it1 != a.laptopi.end(); it1++)
        {
           
            laptopi.insert(std::make_pair<int, std::shared_ptr<Laptop>>(std::remove_reference<int&>::type(it1->first), std::make_shared<Laptop>(it1->first, (it1->second)->DajNaziv(), (it1->second)->DajKarakteristike())));
        }
    };
    Administracija(Administracija &&a)
    {
        for(auto it(a.studenti.begin()); it != a.studenti.end(); it++)
        {
            std::shared_ptr<Student> s(it->second);
            it->second = nullptr;
            studenti.insert(std::make_pair<int, std::shared_ptr<Student>>(std::remove_reference<int&>::type(s->DajIndeks()), std::remove_reference<std::shared_ptr<Student>&>::type(s)));
            s = nullptr;
        }
        for(auto it1(a.laptopi.begin()); it1 != a.laptopi.end(); it1++)
        {
            std::shared_ptr<Laptop> l(it1->second);
            it1->second = nullptr;
            laptopi.insert(std::make_pair<int, std::shared_ptr<Laptop>>(std::remove_reference<int&>::type(l->DajEvidencijskiBroj()), std::remove_reference<std::shared_ptr<Laptop>&>::type(l)));
            l = nullptr;
        }
    };
    /*~Administracija()
    ...
    void RegistrirajNovogStudenta(int indeks, std::string GodStud, std::string ImePrez, std::string Adr, std::string BrojTel)
    {
         ...
         std::shared_ptr<Student> s = std::make_shared<Student>(indeks, GodStud, ImePrez, Adr, BrojTel);
         studenti.insert(std::make_pair<int, std::shared_ptr<Student>>(std::remove_reference<int&>::type(indeks), std::remove_reference<std::shared_ptr<Student>&>::type(s)));
         s = nullptr;
    };
    void RegistrirajNoviLaptop(int broj, std::string naz, std::string kar)
    {
        ...
        std::shared_ptr<Laptop> l = std::make_shared<Laptop>(broj, naz, kar);
        laptopi.insert(std::make_pair<int, std::shared_ptr<Laptop>>(std::remove_reference<int&>::type(broj), std::remove_reference<std::shared_ptr<Laptop>&>::type(l)));
        l = nullptr;
    };
    ...
    Student NadjiStudenta(int indeks) const
    {
        auto iter(studenti.find(indeks));
        if(iter == studenti.end()) throw std::domain_error("Student nije nadjen\n");
        std::shared_ptr<Student> s = std::make_shared<Student>(iter->second->DajIndeks(), iter->second->DajGodinuStudija(), iter->second->DajImePrezime(), iter->second->DajAdresu(),iter->second->DajTelefon());
        return *s;
    };
    ...
    Laptop NadjiLaptop(int broj) const
    {
        auto iter(laptopi.find(broj));
        if(iter == laptopi.end()) throw std::domain_error("Laptop nije nadjen\n");
        std::shared_ptr<Laptop> l = std::make_shared<Laptop>(broj, iter->second->DajNaziv(), iter->second->DajKarakteristike());
        return *l;
    };
