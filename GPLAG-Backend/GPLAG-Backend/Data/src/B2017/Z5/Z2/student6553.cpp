...
public:
 ...
 typedef std::shared_ptr<Student> pametni_s;
typedef std::shared_ptr<Laptop> pametni_l;
class Administracija
{
    std::map<int, pametni_s> studenti;
    std::map<int, pametni_l>laptopi;
public:
    ...
    Administracija(const Administracija& v) {
        try {
            ...
            for(auto it=v.studenti.begin(); it!=v.studenti.end(); it++) {
                studenti.emplace(std::make_pair((it->first), std::make_shared<Student>(*(it->second))));}
            }
            catch(std::bad_alloc) {
            ...
            try {
                ...
                for(auto it=v.laptopi.begin(); it!=v.laptopi.end(); it++) {
                    laptopi.emplace(it->first, std::make_shared<Laptop>(*(it->second)));
                }}
                catch(std::bad_alloc) {
...
void RegistrirajNovogStudenta(int broj_indeksa, std::string godina_studija, std::string ime_i_prezime, std::string adresa,
             std::string telefon){
                 ...
                 studenti.emplace(broj_indeksa, std::make_shared<Student>(broj_indeksa, godina_studija, ime_i_prezime, adresa, telefon));
             }
