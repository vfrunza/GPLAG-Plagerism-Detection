...
public:
    ...
    Administracija &operator =(const Administracija &a){
        ...
        for(auto i=m2.begin(); i!=m2.end(); i++) delete i->second;
        m1.clear(); m2.clear();
        for(auto i=a.m1.begin(); i!=a.m1.end(); i++) m1.insert(make_pair(i->first, new Student(i->second->DajIndeks(),i->second->DajGodinuStudija(),i->second->DajImePrezime(), i->second->DajAdresu(),i->second->DajTelefon())));
        for(auto i=a.m2.begin(); i!=a.m2.end(); i++) m2.insert(make_pair(i->first, new Laptop(i->second->DajEvidencijskiBroj(), i->second->DajNaziv(), i->second->DajKarakteristike())));
        return *this;
    ...
    void RegistrirajNovogStudenta(int ind,string g, string iip, string adr, string brtel){
        ...
        m1[ind]=new Student(ind, g, iip, adr, brtel);
    }
    ...
    void PrikaziZaduzenja(){
        auto i=find_if(m2.begin(), m2.end(), [](pair<int,Laptop*> lap){
    ...
    int NadjiSlobodniLaptop(){
        auto it=find_if(m2.begin(), m2.end(), [](pair<int,Laptop*> lap){
            return !(lap.second->DaLiJeZaduzen());});
