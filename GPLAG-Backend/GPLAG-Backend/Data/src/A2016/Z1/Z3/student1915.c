...
else if(t=='s' || t=='S')
{
	...
	else if((s<p && b<p && c<p && v<p) || (s<=p && b<=p && c<=p && v<=p && ps<pp && pv<=pp && pc<=pp && pb<=pp))
	{ n=(p/u)*100;
	printf("\nNajpopularnija boja je plava (%.2f%%).",n);
	}
	else if((s<b && p<b && c<b && v<b) || (s<=b && s>=p && c<=b && v<=b && ps>pp && pb<ps && pc<ps && pv>ps))
	{ n=(b/u)*100;
	printf("\nNajpopularnija boja je bijela (%.2f%%).",n);
	}
	else if((s>p && b<s && c<s && v<s) || (s>=p && b<=s && c<=s && v<=s && ps>pp && pb<ps && pc<ps && pv<ps))
	{ n=(s/u)*100;
