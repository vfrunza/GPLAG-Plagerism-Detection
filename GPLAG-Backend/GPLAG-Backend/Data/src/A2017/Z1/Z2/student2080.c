...
int main() {
 ...
 max = max*100;
	...
	if((fabs(t1-t2)<e)||(fabs(t1-t3)<e)||(fabs(t2-t3)<e)) {
		...
		printf("Jednakokraki\n");
		printf("Najveci ugao ima %i stepeni, %i minuta i %i sekundi.", d, m, s);
		return 0;
	}
	
	if (t1 != t2 && t2!=t3 && t3!=t1 && t1>t2 && t2>t3) {
		...
		else if (t1 != t2 && t2!=t3 && t3!=t1 && t3>t2 && t2>t1) {
