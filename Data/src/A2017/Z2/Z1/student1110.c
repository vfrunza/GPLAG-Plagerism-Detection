...
int main() 
{
	...
	for(i=0; i<N; i++)
	{
		for(j=0; j<N; j++)
		{
			...
			else if((i==j && i>N/2 && i<N-1 && i<=N-N/4 && i>=N-N/4-1) || (i+j==N*2-N/2-1 && i>N/2 && i<N-1 && i<=N-N/4 && i>=N-N/4-1)) printf("+");
			else if(j>N/2 && j<N-1 && j<=N-N/4 && j>=N-N/4-1 && i>=N/2) printf("|");
			else if(j>=N/2 && j<=N-1 && i>N/2 && i<N-1 && i<=N-N/4 && i>=N-N/4-1) printf("-");
			else printf(" ");
		}
		printf("\n");
	}
