...
int trebalimijenjati(char *tekst,char *rijec){
	char *pocetak=tekst;
...
void zamijeni(char* tekst,char* rijec,char* novarijec){
	...
	while(*tekst!='\0'){
		...
		if(*tekst!='\0' && (tekst==temp || *(tekst-1)== ' ') && trebalimijenjati(tekst,rijec)){
...
char* zamjena_rijeci(char* tekst, char** rijeci, char** zamjene, int broj_rijeci){
	...
	for(i=0;i<broj_rijeci;i++){
		...
		while(*tekst!='\0'){
			...
			if(trebalimijenjati(tekst,rijeci[i])){
